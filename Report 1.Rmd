...
```{r as.is= FALSE}
  ...
  number_of_words <- length(tidy_corp$word)
  distinct_words <- length(tidy_corp_summary$word)
...
corp_bigram_count <- corp_bigram_count %>% 
                    arrange(rank(desc(n))) %>% 
                    bind_tf_idf(bigram,orig,n)desc
bigram_graph <- corp_bigram_count %>% 
                filter(rank(desc(tf)) <= 10) %>%
                graph_from_data_frame()
g1 <-  corp_bigram_count %>% filter(rank(desc(n))<= 40) %>%
  ggplot(aes(x=reorder(bigram,myorder), tf)) +
  geom_point() +
  xlab(NULL) +
  ylab("Frequency") +
  coord_flip() +
