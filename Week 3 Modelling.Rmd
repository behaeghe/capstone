...
corp_trigram_model <-  unnest_tokens(sample_n(corp,1000),ngram,text,token="ngrams",n=3) %>%
                      count(ngram,sort=TRUE) %>% 
                      separate(ngram, c("word1","word2","word3"),sep=" ") %>%
                      mutate(bigram=paste(word1,word2," ")) %>%
...
corp_fourgram_model <-  unnest_tokens(sample_n(corp,1000),ngram,text,token="ngrams",n=4) %>%
                      count(ngram,sort=TRUE) %>% 
